...
{
  $sql = <<<SQL
    SELECT SenhaHash FROM PESSOA,FUNCIONARIO
    WHERE PESSOA.email = ? AND PESSOA.codigo = FUNCIONARIO.Codigo
    SQL;
...
function checkLogged($pdo)
{
  ...
  $sql = <<<SQL
    ...
    FROM FUNCIONARIO INNER JOIN PESSOA ON PESSOA.CODIGO = FUNCIONARIO.CODIGO
    WHERE PESSOA.email = ?
    SQL;
